resource "azurerm_availability_set" "as-workers" {
...
resource "azurerm_public_ip" "pip_workers" {
...
resource "azurerm_network_interface" "nic_workers" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.pip_workers[count.index].id
  }
...
resource "azurerm_linux_virtual_machine" "k8s_workers" {
  ...
  availability_set_id = azurerm_availability_set.as-workers.id
